...
if (isset($_POST['submit'])) {
  ...
  $sqlPass = "SELECT * FROM customer WHERE username = '$username' ";
  $resultGetPass = mysqli_query($conn, $sqlPass);
  if (mysqli_num_rows($resultGetPass) == 0) {
    $userAvailable = '<span class="text-danger">Tên đăng nhập không tồn tại</span>';
    $resultUser = "border-danger";
  }
  $rowGetPass = mysqli_fetch_assoc($resultGetPass);
  if ($password == "") {
    $resultPass = "border-danger";
    $passAvailable = '<span class="text-danger">Mật khẩu không hợp lệ</span>';
  } else if (isset($rowGetPass['password']) && $rowGetPass['password'] != $password) {
    $resultPass = "border-danger";
    $passAvailable = '<span class="text-danger">Sai mật khẩu</span>';
  }
